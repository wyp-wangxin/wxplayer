        ...
        else if(pFormatCtx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
        {
            if(video == NULL)
            {
                ...
                if(num != 0 && den != 0)
                {
                    ...
                    video->defaultDelayTime = 1.0 / fps;//计算视屏的帧率
                }
            }
        }
    }
...
void WlFFmpeg::start() {
    ...
    {
        LOGE("supportvideo 当前设备支持硬解码当前视频");
        ...
        if(av_bsf_alloc(bsFilter, &video->abs_ctx) != 0)//初始化过滤器上下文
        {
            supportMediacodec = false;
            goto end;
        }
        if(avcodec_parameters_copy(video->abs_ctx->par_in, video->codecpar) < 0)//拷贝添加解码器属性
        {
            supportMediacodec = false;
        ...
        if(av_bsf_init(video->abs_ctx) != 0)//初始化过滤器上下文
        {
            supportMediacodec = false;
            av_bsf_free(&video->abs_ctx);
            video->abs_ctx = NULL;
        ...
        video->abs_ctx->time_base_in = video->time_base;//时间基从新设置一下
    }
...
void WlFFmpeg::release() {
    if(LOG_DEBUG)
    {
        LOGE("WlFFmpeg::release playstatus->exit : %d",playstatus->exit);
    }
    playstatus->exit = true;
